<script>
  ...
  onMount(() => {
    try { if (hasStream()) { dispatch('close'); } } catch {}
  });
...
{/if}
<style>
  .screen-modal-backdrop { position: fixed; inset: 0; background: var(--wph-backdrop, rgba(0,0,0,0.5)); display: grid; place-items: center; z-index: 1000003; }
  .screen-modal { width: min(520px, calc(100vw - 24px)); border-radius: 16px; background: var(--wph-surface, #121318); color: var(--wph-text, #fff); border: 1px solid var(--wph-border, rgba(255,255,255,0.12)); box-shadow: 0 16px 36px rgba(0,0,0,0.5); padding: 14px 16px 12px; font-size: 14px; line-height: 1.45; font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }
  .header { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
  .icon { width: 36px; height: 36px; display: grid; place-items: center; border-radius: 50%; background: var(--wph-primary, #f05123); color: var(--wph-onPrimary, #fff); }
  .title { font-size: 16px; font-weight: 700; color: var(--wph-text, #fff); }
  .body { color: var(--wph-muted, #e9e9e9); }
  ...
  .btn-primary { background: var(--wph-primary, #f05123); color: var(--wph-onPrimary, #fff); border-color: var(--wph-primary, #f05123); box-shadow: 0 6px 18px var(--wph-primaryGlow, rgba(240,81,35,0.35)); }
  .btn-primary:hover { filter: brightness(1.05); }
  .btn-secondary { background: var(--wph-surface, rgba(255,255,255,0.95)); color: var(--wph-text, #222); border-color: var(--wph-border, rgba(0,0,0,0.1)); box-shadow: 0 6px 18px rgba(0,0,0,0.25); }
  .btn-secondary:hover { filter: brightness(1.03); }
  .icon svg.on { filter: drop-shadow(0 0 4px var(--wph-primaryGlow, rgba(240,81,35,0.4))); }
