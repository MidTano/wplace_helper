<script>
  import { t, toggleLang, lang } from '../i18n';
  $: current = $lang;
  function onClick() { toggleLang(); }
</script>

<button class="tm-fab tm-tip" on:click={onClick} aria-label={t('btn.language')} data-label={`${t('btn.language')}: ${current === 'ru' ? t('lang.ru') : t('lang.en')}`}>
