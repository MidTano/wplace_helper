<script>
  ...
  function onMoveEvt(e) {
    try { on = !!(e?.detail?.on); } catch {}
  }
  onMount(() => {
    try { on = isMoveMode && isMoveMode(); } catch {}
    try { window.addEventListener('wplace:moveMode', onMoveEvt); } catch {}
  });
  onDestroy(() => { try { window.removeEventListener('wplace:moveMode', onMoveEvt); } catch {} });
...
<button class="tm-fab tm-tip {on ? 'tm-primary' : ''}" type="button" on:click={toggle} aria-pressed={on} aria-label={L_move} data-label={L_move}>
